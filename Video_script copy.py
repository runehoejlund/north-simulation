...
def animate(i):
    ax.clear()
    #ax.contourf(np.transpose(data['vort'][i, :, 0, :]), 100, cmap = 'jet', vmin=-0.50, vmax=0.50)
    ax.contourf(np.transpose(data['n'][i, :, 0, :]), 100, cmap = 'hot', vmin=5, vmax=11)
...
# Call animate method
ani = animation.FuncAnimation(fig, animate, frames=323, interval=100, blit=False)

ani.save('plots/animation_Te.gif', fps = 20)
